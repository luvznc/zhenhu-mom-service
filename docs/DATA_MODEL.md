...
| region_code | 城关/七里河/安宁/西固等服务圈编码 |
| local_preferences | 本地护理师、语言、饮食和环境记录偏好 |
...
| home_region / service_regions | 常驻服务圈与可服务区域，不保存公开精确住址 |
| languages / local_service_tags | 普通话/方言、本地饮食沟通、环境记录等标签 |
...
| region_snapshot | 签约时区域、脱敏距离、通勤和地域偏好快照 |
...
| --- | --- |
| code / name | 服务圈编码与名称 |
| coverage_areas | 可服务街道/片区列表 |
| active_caregiver_count | 当前可用护理师数量 |
| default_radius_km | 默认优先匹配半径 |
| supply_updated_at | 供给数据更新时间 |
| regional_rules | 跨区、夜间交通、替补与响应规则版本 |
